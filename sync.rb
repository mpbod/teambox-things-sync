...
Timeout::timeout(40) {
  ...
  client.projects.each do |project|
    ...
    person_id = find_person_id(project_people, client.account.id)
    
    unless person_id.nil?
      # update tasks marked as done in Things
      client.tasks.each do |task|
        things_todo = Things::Todo.find(task.name)
        if !things_todo.nil? && things_todo.completed? && [0,1].include?(task.status)
          ...
          puts "#{task.name} at remote set as done"
        end
      end
      ...
      client.tasks.each do |task|
        ...
        if task.assigned_id == person_id
          ...
          new_todo = find_or_create_todo_in_things(task.name)
          #update only those that aren't completed in both local and remote
          unless ![0,1].include?(task.status) && new_todo.completed?
            notes = "Don't edit this field!\n"
            notes += task_url(config, project.name, task.task_list_id, task.id)
            new_todo.project = find_or_create_project_in_things(project.name)
            new_todo.notes = notes
            unless task.due_on.nil?
              new_todo.due_date = Time.parse(task.due_on)
            end
            ...
            new_todo.tag_names = task_list_cache[task.task_list_id]
            new_todo.save
            puts "\"#{task.name}\" have been saved in Things.app"
          end
        end
      end

      # maybe add task from things to teambox app
